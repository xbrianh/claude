"""Shell integration tests for the ghgremlin shim + gremlins package.
...
def _run_shim_directly(sh, *args, timeout=30):
    ...
    The shim execs `python -m gremlins.cli gh "$@"`, so this exercises the
...
def test_ghgremlin_unknown_pipeline_flag_rejected(tmp_path):
    ...
    # argparse emits to stderr; gremlins.cli's die() also goes to stderr.
    assert "unrecognized" in r.stderr.lower() or "error" in r.stderr.lower()


def test_ghgremlin_shim_forwards_to_gremlins_cli(tmp_path):
    """ghgremlin.sh shim is a thin exec into `python -m gremlins.cli gh`.

    Invoke it with no args and confirm the orchestrator emits its own
    usage error (proving control reached gremlins.cli gh's argparse).
    """
    sh = setup_shell_env(tmp_path, with_gh=True, with_origin=True)
