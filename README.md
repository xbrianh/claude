...
skills/
  ...
  localgremlin/       # /localgremlin: thin shims that exec into `gremlins`
  ...
  gremlins/           # /gremlins: thin shim into `gremlins fleet`; on-demand status + stop/rescue/rm/close/land subcommands
  handoff/            # /handoff: thin shim into `gremlins handoff`; foreground chain-step decision agent
...
- [`/ghgremlin`](skills/ghgremlin/SKILL.md) — GitHub-issue-driven counterpart to `/localgremlin`: runs plan → implement → PR creation → Copilot + Claude review → address via the `gremlins` CLI (`gremlins gh`). [`skills/ghgremlin/ghgremlin.sh`](skills/ghgremlin/ghgremlin.sh) is a thin shim kept for manual invocation.
...
The gh pipeline (driven by `gremlins gh`) chains them: `/ghplan` → implement → `/ghreview` (Copilot + Claude) → `/ghaddress`, producing a merged-ready PR from a single instruction.
...
- Dispatches per kind to the right `gremlins` subcommand: `localgremlin` → `local`, `ghgremlin` → `gh`, `bossgremlin` → `boss`.
