...
- `land <id>`: land a finished gremlin onto its target branch, then clean up. Preserves the state directory (`plan.md`, review artifacts, run log, `state.json`) so you can inspect the gremlin's record after merge — use `rm <id>` for full cleanup. Behavior depends on gremlin kind:
