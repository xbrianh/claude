...
- `rescue <id>` (interactive, default): two-phase diagnose-then-resume for a dead or stalled gremlin. **Phase A (foreground):** reads the failure log and existing artifacts, then spawns a `claude -p` agent **inline** in the gremlin's worktree to diagnose and fix the underlying issue. The agent's prompt requires it to write a marker file at `<state-dir>/artifacts/rescue-<ts>.done` containing `{"status": "fixed" | "transient" | "unsalvageable", "summary": "..."}` before exiting — this is the agent → wrapper handoff signal, used by both interactive and headless rescue. Ctrl-C to abort — state is preserved, no marker is required, no handoff happens. **Phase B (background):** the wrapper reads the marker after Phase A and acts on its `status`:
