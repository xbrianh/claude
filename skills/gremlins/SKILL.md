...
- `rescue --headless <id>`: same two-phase flow, run end-to-end with no TTY and no user input — for unattended callers (humans walking away, future bossgremlin). Differences from interactive:
  ...
  - **Phase A contract:** runs `claude -p --permission-mode bypassPermissions --output-format text` with stdin closed and a wall-clock timeout (default 1800s, override via `HEADLESS_RESCUE_TIMEOUT_SECS`). The headless prompt points the agent at the pipeline source actually executing under `~/.claude/skills/`, the gremlin's worktree, the product repo at the current default branch via `origin/HEAD` (so the agent can see fixes that landed after the chain started), and — for boss children — the parent boss's state dir (`boss_state.json`, rolling plan documents, sibling child plans). It instructs the agent to write a marker JSON file at `<state-dir>/artifacts/rescue-<ts>.done` containing `{"status": "fixed" | "transient" | "structural" | "unsalvageable", "summary": "..."}`. `summary` must be a string (no objects/arrays); the wrapper collapses whitespace and caps length so a chatty diagnosis can't injection-newline the boss log. The four verdicts are deliberately separate; the wrapper reads the marker and acts accordingly:
    - `fixed` → the agent edited `state.json` and/or files inside the worktree to address the root cause; proceed to Phase B. Pipeline source under `~/.claude/skills/` is read-only for headless rescues (it would be clobbered by the next `sync.sh pull`); fixes that belong there must come back as `structural` instead.
    - `transient` → no edits needed; the failure was a flake (network, tool timeout, retriable infra) or a fix has already landed elsewhere (in `main`, in a synced `~/.claude/skills/` file) that the chain's pre-fix base ref doesn't see. Proceed to Phase B as a relaunch-only attempt.
    - `structural` → the agent recognized a real bug in the pipeline source (`~/.claude/skills/<kind>/*.sh`, `~/.claude/skills/_bg/*.sh`, etc.) or in a sibling artifact (e.g. a malformed child plan under the parent boss's state dir) that requires a human edit elsewhere. The chain *can* be salvaged, but not by this rescue. Write `bail_reason=structural`, store the agent's diagnosis in `bail_detail`, and exit non-zero. Distinct from `unsalvageable`: an operator should look at the named pipeline file or sibling plan, not write off the run.
    - `unsalvageable` → reserved for genuinely unrecoverable states (corrupted state dir, missing worktree, conflicting git state with no clean rewind). Write `bail_reason=unsalvageable`, exit non-zero. Should be rare; if the agent picks `unsalvageable` because the fix isn't in `state.json`, it almost certainly meant `structural`.
