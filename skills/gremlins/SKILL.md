...
- `rescue --headless <id>`: same two-phase flow and same marker contract as interactive, but run end-to-end with no TTY and no user input — for unattended callers (humans walking away, future bossgremlin). Differences from interactive:
  ...
  Each rescue attempt — interactive or headless, success or bail (Ctrl-C aborts excepted) — also writes a Markdown report to `<state-dir>/rescue-<UTC-ts>.md` capturing the attempt number, gremlin context, diagnosis verdict + summary, and relaunch outcome. Multiple attempts accumulate (the timestamp keeps each filename unique). Reports are listed in their own section under the drill-in view (`/gremlins <id>`).
