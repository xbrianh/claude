...
- `rescue --headless <id>`: same two-phase flow, run end-to-end with no TTY and no user input — for unattended callers (humans walking away, future bossgremlin). Differences from interactive:
  ...
  - **Phase A contract:** runs `claude -p --permission-mode bypassPermissions --output-format text` with stdin closed and a wall-clock timeout (default 1800s, override via `HEADLESS_RESCUE_TIMEOUT_SECS`). The headless prompt points the agent at the pipeline source actually executing under `~/.claude/skills/`, the gremlin's worktree, the product repo at current `main` (so the agent can see fixes that landed after the chain started), and — for boss children — the parent boss's state dir (`boss_state.json`, rolling plan documents, sibling child plans). It instructs the agent to write a marker JSON file at `<state-dir>/artifacts/rescue-<ts>.done` containing `{"status": "fixed" | "transient" | "structural" | "unsalvageable", "summary": "..."}`. The four verdicts are deliberately separate; the wrapper reads the marker and acts accordingly:
