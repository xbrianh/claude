...
- `rescue --headless <id>`: same two-phase flow, run end-to-end with no TTY and no user input — for unattended callers (humans walking away, future bossgremlin). Differences from interactive:
  ...
  - **Phase A contract:** runs `claude -p --permission-mode bypassPermissions --output-format text` with stdin closed and a wall-clock timeout (default 1800s, override via `HEADLESS_RESCUE_TIMEOUT_SECS`). The headless prompt gives the agent a curated view of the pipeline source it's running under (`~/.claude/skills/<kind>/*.sh`, `~/.claude/skills/_bg/*.sh`), the parent boss's state dir for boss children (rolling plan, child plans, prior outcomes), and the product repo's `main` ref so it can recognize fixes that landed after the chain started. The agent writes a marker JSON file at `<state-dir>/artifacts/rescue-<ts>.done` containing `{"status": "fixed" | "transient" | "structural" | "unsalvageable", "summary": "..."}`. The wrapper reads the marker:
    - `fixed` → agent edited `state.json` to address the root cause; proceed to Phase B. Reserved for state / pipeline-arg fixes — pipeline-source fixes go through `structural`, since direct edits under `~/.claude/skills/` are invisible to the user's repo and would be overwritten on the next `sync.sh pull`.
    - `transient` → no edit needed; the failure has an external cause already resolved (flake, race, fix landed on `main`). Proceed to Phase B.
    - `structural` → agent identified a real bug in pipeline source or in a sibling artifact (e.g. a child plan) that it can name but cannot safely fix from the rescue context. Write `bail_reason=structural`, `bail_detail=<summary>`, exit non-zero. Distinct from `unsalvageable`: the chain *can* be salvaged by a human edit; the agent is just not the right actor. Operator should fix the named file and re-run / resume.
    - `unsalvageable` → genuinely unrecoverable state (corrupted state dir, missing worktree, conflicting git state with no clean rewind). Write `bail_reason=unsalvageable`, exit non-zero. Should be rare.
