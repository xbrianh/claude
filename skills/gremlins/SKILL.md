---
name: gremlins
description: On-demand status of background gremlins launched by /localgremlin and /ghgremlin. Reads every ~/.local/state/claude-gremlins/<id>/state.json on the machine and prints one line per active gremlin with its kind, current stage, liveness (running / stalled / dead), description, and age. Use to check progress, spot crashed gremlins, acknowledge (hide) finished ones, stop a running gremlin, or rescue a dead/stalled one. Not a project filter by default — set --here to restrict to the current repo.
argument-hint: [stop|rescue|rm <id>] [--here] [--ack <id>] [--ack-all] [--running] [--dead] [--stalled] [--kind local|gh] [--since <dur>] [--recent [N]] [--watch [sec]] [<id-prefix>]
allowed-tools: Bash(~/.claude/skills/gremlins/gremlins.py:*)
---

You are running the `gremlins` status command. It reads the persistent state under `~/.local/state/claude-gremlins/` (or `$XDG_STATE_HOME/claude-gremlins/` if `XDG_STATE_HOME` is set) and summarizes every active (not-yet-acknowledged) background gremlin across every project on this machine.
...
- `ID` — short (6-hex) gremlin identifier. Use it with `--ack <id>` to mark a dead gremlin acknowledged (which hides it from future runs of this command).
...
- `--ack <id>`: mark a finished/dead gremlin as acknowledged (hides it from subsequent lists). Accepts full id or a substring.
- `--ack-all`: acknowledge every dead/finished gremlin.
...
- `--recent [N]`: show recently-finished (`dead:*`) gremlins started within N hours (default 24), including acknowledged ones. Mutually exclusive with `--running`/`--dead`/`--stalled`. Composes with `--here` and `--kind`.
...
- `rescue <id>`: diagnose and resume a dead or stalled gremlin. Reads the failure log and existing artifacts, then spawns a `claude -p` agent **inline** (synchronous, output visible in terminal) in the gremlin's worktree. The agent diagnoses the failure, fixes the underlying issue, and completes the remaining stages. Runs synchronously — Ctrl-C to abort. After a successful rescue, run `/localland` to merge the branch if satisfied. Note: if the gremlin is still running, `rescue` will refuse and suggest using `stop` first.
- `rm <id>`: delete a dead or finished gremlin's state directory from disk (the log, plan, reviews, and `state.json`). Refuses with an error if the gremlin is still running or stalled — use `stop` first, then `rm`. This is permanent; use when you want to fully clean up a gremlin rather than just hide it with `--ack`.
