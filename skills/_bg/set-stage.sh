...
# gremlin's state.json. Called by gremlin scripts at each stage boundary
# so `/gremlins` and the session-summary hook can report where a gremlin is.
#
# Usage: set-stage.sh <gr_id> <stage> [sub_stage_json]
#
# Fails silently on any error — stage bookkeeping must never break a running
# gremlin. Callers should also guard on [[ -n "${GR_ID:-}" ]] so direct
# invocations of the gremlin (outside the launcher) are no-ops.
set -u
